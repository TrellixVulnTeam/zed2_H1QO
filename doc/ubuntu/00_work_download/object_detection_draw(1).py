...
def object_detection_inference_result(path,detector,category_index,th=0.5):
    img = load_image(path)
...
def draw_bbox_on_image_inferenced_result(path_img,path_category_index,path_mod,path_img_out,th=0.5):
    category_index = np.load(path_category_index, allow_pickle='TRUE').item()
    detector = hub.load(path_mod)
    img, detection_dt= object_detection_inference_result(path_img, detector, category_index, th=th)
    image = np.array(img[0])
    image_with_boxes = draw_boxes(
        image, detection_dt["detection_boxes"],
        detection_dt["detection_classes_name"], detection_dt["detection_scores"])
...
if __name__ == "__main__":
  ...
  draw_bbox_on_image_inferenced_result(path, path_category_index, path_mod, path_o, th=0.5)