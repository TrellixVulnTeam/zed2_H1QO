...
def load_image(fn, input_size, t="npy"):
    # color = cv2.imread(fn)
...
def load_image_small(img, input_size, t="npy"):
    if t == 'npy':
        color, color_org = load_image_from_npy(img)
    elif t == 'png':
...
def load_model_seg(tflite_path):
...
def main():
    ...
    interpreter, input_details, input_size = load_model_seg(tflite_path)
...
def semantic_segmentation_from_npy_all_file(path, path_mod, path_lable):
    ...
    interpreter, input_details, input_size = load_model_seg(path_mod)
    ...
    for i, fpdiri in enumerate(fpdir):
        ...
        path_o = f'{fdir}/human_segmentation'
        ...
        path_o = f'{fdir}/human_segmentation_crop'
        cv2.imwrite(path_o + '.png', np.array(out_img))


def load_labels_info_landscape():
    labels_list = [
        'road',
        'sidewalk',
        'building',
