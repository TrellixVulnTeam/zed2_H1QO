...
def main():
    ...
    mesh_frame = o3d.geometry.TriangleMesh.create_coordinate_frame(size = 0.5, origin = [0,0,0]) #original camera frame
...
def downsample_clouds(pcds, voxel_size):
    for i in range(len(pcds)):
        pcds[i] = pcds[i].voxel_down_sample( voxel_size)
    return pcds
...
def remove_clouds_outliers(pcds, num_points, radius, ratio=0):
    for i in range(len(pcds)):
        ...
        cl, ind = pcds[i].remove_statistical_outlier(nb_neighbors=num_points,
                                                            std_ratio=ratio)
        pcds[i] = pcds[i].select_down_sample(ind)
        # if ratio > 0:
