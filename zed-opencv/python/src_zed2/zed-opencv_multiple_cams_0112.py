...
basePath = "./data/"
stop_signal = False
sl_frame=sl.REFERENCE_FRAME.WORLD
# sl_frame=sl.REFERENCE_FRAME.FRAME_WORLD
...
def get_translate(cams, index, cnt_r=10):
    ...
    # https://github.com/stereolabs/zed-examples/blob/master/tutorials/tutorial%204%20-%20positional%20tracking/python/positional_tracking.py
    global sl_frame
    state =zed.get_position(zed_pose, sl_frame)
...
def get_cam_data(cams, index, take_cam_id):
    ...
    # get pose data
    pose_lst = get_pos_dt(cams, index)
    ...
    # get transform matrix
    posetransform = get_pose_transform_matrix(cams, index)
    np.savetxt(f'{fd_cam}/trans_m_frame_world.csv', posetransform)
    # get pose data
    pose_lst = get_pos_dt(cams, index)
    np.savetxt(f'{fd_cam}/pose_ori_frame_world.csv', pose_lst)

    imu_pose = get_imu_pose(cams, index)
    np.savetxt(f'{fd_cam}/pose_imu_frame_world.csv', imu_pose)

    trans = translations_quaternions_to_transform(pose_lst)
    np.savetxt(f'{fd_cam}/trans_qua_frame_world.csv', trans)
...
def main():
    ...
    init = sl.InitParameters(
        camera_resolution=sl.RESOLUTION.HD720,## Use HD720 video mode (default fps: 60)
