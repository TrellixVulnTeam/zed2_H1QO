...
def get_good_matches_pts(goodMatches,kp1,kp2,imgdic,cam):
    ...
    for i, goodMatch in enumerate(goodMatches):
        ...
        pts1=make_obj(p1, d1)
        pts2=make_obj(p2,d2)
        if np.isnan(np.mean(pts1)) or np.isnan(np.mean(pts2)):
            continue
...
def main_func(keymatchfun,ext,cam,imgdic):
    ...
    cloud1_gm=cloud1.transform(pose_mat)
    ...
    # open3d.io.write_point_cloud(f'{bp}pcd_org1.ply', cloud1)
    # points1=np.array(cloud1.points)
    # colors1=np.array(cloud1.colors)
    # points1t = pc.warp3d(points1.T, estPose)
    #
    # cloud1t2 = make_pcd(points1t.T, colors1)
    # cloud2t=cloud2.transform(pose_mat)
    cloud1t2=cloud1.transform(estPose)
    open3d.io.write_point_cloud(f'{bp}{ext}_pcd_trans_1_t.ply', cloud1_gm)
    open3d.io.write_point_cloud(f'{bp}{ext}_pcd_trans_1_t2.ply', cloud1t2)
    # # open3d.io.write_point_cloud('pcd_trans_2t.ply', cloud2t)
    pcd=merge_points_cloud([cloud1_gm,cloud2])
...
imgdic = EasyDict({})
imgdic.rgb1=cv2.imread(f"{bp}/cam0/image.png")
imgdic.rgb2=cv2.imread(f"{bp}/cam1/image.png")
imgdic.dpt1=np.load(f"{bp}/cam0/depth.npy")
imgdic.dpt2=np.load(f"{bp}/cam1/depth.npy")

for matchfun, ext in keyfuns:
    main_func(matchfun, ext,cam_zed2_21888201_left_hd1080,imgdic)
