...
def convert_zed2pcd_to_ply_moving(zed_pcd):
...
def get_moving_object_mask(img_path_moving,img_path_average):
...
def get_pcd_by_mask(pcd_path,img_msk):
    zed_pcd = np.load(pcd_path)
    msk1=np.expand_dims(img_msk[:, :, 0],axis=2)
    img_msk4=np.concatenate([msk1,msk1,msk1,msk1],axis=2)
    k = zed_pcd[:, :, 0] * img_msk[:, :, 0]
    zed_pcd_out=zed_pcd*img_msk4
    ply=convert_zed2pcd_to_ply_moving(zed_pcd_out)
    return ply
def get_moving_object_ply_by_mask(img_path_moving,img_path_average,pcd_path):
    img_msk,img=get_moving_object_mask(img_path_moving,img_path_average)
    ply=get_pcd_by_mask(pcd_path,img_msk)
...
def main_image_mask_ply():
    ...
    path_ply='C:/00_work/05_src/data/20201124/202011161_sample/ply'
    camlst=['cam0', 'cam1', 'cam2']
    same_time_cam_lst=get_same_time_image_from_cameras(pathi,camlst=camlst,th=200000)
    ...
    for i, same_times in enumerate(same_time_cam_lst):
        ...
        for j, time_id in enumerate(same_times):
            ...
            ply,img_msk,img=get_moving_object_ply_by_mask(fn_img_moving,fn_img_average,fn_pcd)
...
if __name__ == "__main__":
    ...
    # path = '/home/user1/yu_develop/202011161_sample/image/'
    # patho = '/home/user1/yu_develop/202011161_sample/ssim/'
