...
def get_human_lst_each_cam_2():
    ...
    df_detection = df_detection.replace('/media/user1/Data/data/20201116/',
                                        '/home/user1/yu_develop/202011161_sample/image/')
    for i, cam in enumerate(camlst):
        df = df_detection[df_detection[0].str.contains(cam)]
        # image_average=get_average_image(df)
...
class moving_object_merge:
    ...
    def get_moving_object_ply_by_mask(self, img_path_moving, img_path_average, pcd_path):
        ...
        img_msk = remove_noise_mask(img_msk, 11)
    ...
    def get_absolute_file_paths(self, directory, ext=".npy", fn='image.npy') -> object:
        dt_list=['20201116133246_027942','20201116133925_495618',
                 '20201116133246_072897','20201116133925_659685',
                 '20201116133246_079488','20201116133925_340958']
        fils_list = []
        fn_list = []
        dir_list = []
        for dirpath, _, filenames in os.walk(directory):
            for f in filenames:
                if f.find(fn) >= 0:
                    fnid= os.path.basename(os.path.normpath(dirpath))
                    if fnid in dt_list:
...
def main_image_mask_ply():
    pathi = 'C:/00_work/05_src/data/20201124/202011161_sample/image'
    patho = 'C:/00_work/05_src/data/20201124/202011161_sample/out'
    path_ply = 'C:/00_work/05_src/data/20201124/202011161_sample/fuchi_ply'
    os.makedirs(path_ply,exist_ok=True)
    camlst = ['cam0', 'cam1', 'cam2']
    mob = moving_object_merge()
...
# def check_fuchi(img, x, y):
#     ret=False
#     if img[x,y,0] == 1 and img[x-10,y-10,0]==0:
#         ret=True
#     return ret
def check_fuchi(img, y, x):
...
def remove_noise_mask(img, ksize):
    ...
    print('check_fuchi s',datetime.datetime.now().strftime('%Y%m%d%H%M%S_%f'))
    fuchi_msk=fuchi_deal_msk(img_mask)
    print('check_fuchi e',datetime.datetime.now().strftime('%Y%m%d%H%M%S_%f'))
    return fuchi_msk
