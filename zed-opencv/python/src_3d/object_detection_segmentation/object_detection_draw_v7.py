...
def load_model_obj(path_mod, path_category_index):
...
def object_detection_inference_bbox(path, detector, category_index, th=0.5):
    # img = load_image(path)
    img, color_org = load_image_from_npy(path)
    img_ret = cp.deepcopy(img)
...
# output the bbox crop image to destinition file path
def human_bbox_on_image_inferenced_result(path_img, detector, category_index, th=0.5):
    image, detection_dt = object_detection_inference_bbox(path_img, detector, category_index, th=th)
    bboxes_ret = []
    for bbox, cls in zip(detection_dt["detection_boxes"], detection_dt["detection_classes_name"]):
        if cls == 'person':
            bboxes_ret.append(bbox)
    return bboxes_ret, image


def object_detection_from_npy_all_file(pathi, patho, path_mod, path_category_index):
    ...
    fils_list, fn_list, dir_list = get_absolute_file_paths(pathi, ext=".npy", fn='image.npy')
    ...
    for i, fpdiri in enumerate(fpdir):
        path, fdir = fpdiri
        fdir_out = fdir.replace(pathi, patho)
        os.makedirs(fdir_out, exist_ok=True)
        path_o = f'{fdir_out}/human_detection'
        print(datetime.datetime.now().strftime('%Y%m%d%H%M%S'), "detection:", i, cnt, path)
        detection_dt = draw_bbox_on_image_inferenced_result(path, detector, category_index, path_o, th=0.5)
        dt_result = detection_dt["detection_classes_name"]
        if len(dt_result[dt_result == 'person']) > 0:
            human_dt_lst.append([path, 1])
        else:
            human_dt_lst.append([path, 0])
    return human_dt_lst
