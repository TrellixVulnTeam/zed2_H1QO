...
'''
#参照資料
def computeKeyPointsAndMachesORB(img1,img2):
    # OBR 特徴量検出器を作成する。
...
def get_good_matches_pts(goodMatches,kp1,kp2):
    ...
    _, rVec, tVec = cv2.solvePnP(pts_obj, pts_img, cameraMatrix, distCoeffs=None,
                                 ...
                                 flags=cv.SOLVEPNP_UPNP, useExtrinsicGuess=False, rvec=rvec0, tvec=T0)
                                 # flags=cv.SOLVEPNP_EPNP, useExtrinsicGuess=False, rvec=rvec0, tvec=T0)
...
# kp1,kp2,goodMatches=computeKeyPointsAndMachesORB(rgb1,rgb2)
kp1,kp2,goodMatches=computeKeyPointsAndMachesSift(rgb1,rgb2)
...
open3d.io.write_point_cloud(f'pcd_trans_1_{ext}.ply', cloud1t)
pcd=merge_points_cloud([cloud1t,cloud2])
open3d.io.write_point_cloud(f'pcd_merge_{ext}.ply', pcd)